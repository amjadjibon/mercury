...
- `strategy`: `StrategyRunner` (dispatches events to strategies), `MarketMaker`, `Momentum`, `RsiStrategy`, `ArbitrageStrategy`, technical indicators (`SMA`, `EMA`, `RSI`, `MACD`)
- `risk`: `RiskManager` (position limits, daily loss guard, rate limiting, kill switch)
- `execution`: `OrderManager` (wraps gateway with risk checks), `SimulatedExchange` (for backtest), `ExecutionMetrics`
...
- `cli`: Binary `mercury` — `run`, `replay`, `backtest` subcommands
